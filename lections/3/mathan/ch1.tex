...
\begin{example}
  ...
  $S_n(z) = \sum\limits_0^n z^n = \frac{1-z^{n+1}}{1-z}$.
...
$U_k \geq 0$, $S_n = \sum\limits_{0}^{n} U_k$ - неубывающая последовательность. \\
...
\begin{theorem}
    ...
    \begin{enumerate}
      \item
        ...
        $\sum U_k$ сходится и  \\
        если $\sum U_k$ расходится $\Rightarrow$ $\sum V_k$ расходится.
      \item
...
\begin{theorem}[Признак Даламбера]
  ...
  \begin{enumerate}
    \item Если $\exists q$ такое что:
      $\ \forall k \ \frac{U_{k+1}}{U_k} \leq q < 1$, то $U_k$ сходится
    \item Если $\exists \lim\limits_{k \to \infty} \frac{U_{k+1}}{U_k} = q$, то:
    \begin{itemize}
      \item при $q < 1$ сходится
      \item при $q > 1$ расходится
...
\begin{proof}
  Идея доказательства - сравнение с геометрической прогрессией.
  \begin{enumerate}
    \item
      \begin{itemize}
        \item $k = 0, 1, \dots, n$,
          $U_k = U_0 \cdot \frac{U_1}{U_0} \frac{U_2}{U_1} \cdots
          \frac{U_k}{U_{k-1}}
          < U_0 \cdot q^k$
          \begin{comment}
            $\frac{U_k}{U_{k-1}} < q, \ \forall k$
          \end{comment}
          $q < 1$, тогда $\sum U_0 \cdot q^k$ --- сходящаяся геометрическая
          прогрессия.
        \item $U_k = U_0 \cdot
          \frac{U_1}{U_0}\frac{U_2}{U_1}\cdots\frac{U_k}{U_{k-1}}
          \geq U_0 > 0$
          \begin{comment}
            $\frac{U_k}{U_{k-1}} \geq 1, \ \forall k$
          \end{comment}
          $U_k \not \to 0 \Rightarrow$ не выполняется необходимое условие
          сходимости.
      \end{itemize}
  ...
  \begin{enumerate}
    ...
    \item Если $\exists \lim\limits_{k \to \infty} \sqrt[k]{U_k} = q, (q \geq 0)$,
      ...
      \begin{itemize}
        \item при $q < 1$ сходится
        \item при $q > 1$ расходится
        \item при $q = 1$ нужны дополнительные исследования
      \end{itemize}
  \end{enumerate}
\end{theorem}
\begin{remark}
...
\begin{theorem}
  Если $f(x)$ неотрицательна и убывает на $x \geq 1$, то ряд
  \begin{gather}
    \label{th131:series1}
...
\section{Признак сходимости для знакочередующихся рядов}
...
\begin{proof}
  ...
  k = 1, 2, \dots $$
  Так что выполняется неравенство $S_{2k + 2} \geq S_{2k}$. Кроме того, они
  ограничены сверху:
...
\begin{proof}
  ...
  = (a_1 - a_2) B_1 + (a_2 - a_3) B_2 + \dots + (a_{n-1} - a_n) B_{n-1} + a_n
  B_n$ \\
...
\begin{definition}
  ...
  отображение $a_n$ в $a_n^* = a_{k_n}, \ (n~=~1, 2, \dots)$. \\
