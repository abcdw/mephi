...
\begin{definition}
  Сумма $\sigma(f, T, \xi) := \sum\limits_{j = 1}^k f(\xi')|I_j|$ называется
...
$m = 1$ совпадает. Схожесть опеределний позваляет найти сходные методы о
...
\begin{definition}
  Велечины $s(f, T) = \sum\limits_{j} m_j |I_j|, \ S(f,T) = \sum\limits_{j} M_j
